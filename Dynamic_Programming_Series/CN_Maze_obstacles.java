...
public class CN_Maze_obstacles {

    // IMP
    // Since the answer can be large, print it modulo 10^9 + 7.

    public static int solve(int i, int j, int mod, ArrayList<ArrayList<Integer>> mat)
    {
        // base case
    ...
    // TC -> O(2^(m*n))
    // SC -> O(m-1 + n-1)
    static int mazeObstacles1(int n, int m, ArrayList<ArrayList<Integer>> mat) {
        int mod = (int)(1e9 + 7);
        return solve(n-1, m-1, mod, mat);
    }

    public static int solve1(int i, int j, int mod, ArrayList<ArrayList<Integer>> mat, int[][] dp)
    {
        // base case
        if (i >= 0 && j >= 0 && mat.get(i).get(j) == -1) return 0;
        if (i == 0 && j == 0) return 1;
    ...
    static int mazeObstacles4(int n, int m, ArrayList<ArrayList<Integer>> mat) {
        int mod = (int)(1e9 + 7);
        int[] prev = new int[m];
